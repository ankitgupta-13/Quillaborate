...
const ModalFormChangePassword = ({ onCancel, onSubmit }) => {
  ...
  return (
    <div className="fixed w-full h-full top-0 left-0 flex items-center justify-center z-30">
      <div
        className="absolute w-full h-full bg-gray-900 opacity-60"
        onClick={onCancel}
      ></div>

      {/* Modal Content */}
      <div
        className="bg-soft w-10/12 md:w-3/5 mx-auto my-auto p-6 rounded-xl shadow-2xl z-50 overflow-y-auto"
        style={{ maxHeight: "90vh" }}
      >
        {/* Body */}
        <h1 className="text-base font-medium mb-8 sticky inset-0">
        ...
        <form onSubmit={handleSubmit(onValid)}>
          ...
          {errPass && (
            <p className="text-red-500 mt-3 mb-3 font-medium">
              * Password not match
            </p>
          )}
          ...
          <div className="flex justify-end mt-8">
            ...
            <ButtonSave type="submit" onClick={onSubmit} />
          </div>
        </form>
