...
import { RouteUserRole } from "../routes";
    ...
    return menuList?.map(({ name_var, url_var, icon_var, children }, key) => (
      <div key={key} className="mb-5 font-medium">
        {children && children.length > 0 ? (
          <div>
            <div
              className={`flex items-center justify-between px-6 py-3.5 -mx-2 -my-1 cursor-pointer text-white rounded-sm ${
                activeDrop(url_var) || menuActive === url_var
                  ? "bg-red-700 border-white"
                  : ""
              } border-l-8 border-transparent hover:border-white hover:bg-red-900`}
            ...
            >
              ...
              {activeDrop(url_var) || menuActive === url_var ? (
                <i className="ri-arrow-drop-down-line text-lg" />
              ) : (
                <i className="ri-arrow-drop-right-line text-lg" />
              )}
            </div>
            <ul
              className={`${
                menuActive?.includes(url_var)
                  ? "py-1 ml-5 transition duration-500 ease-in-out transform hover:scale-105"
                  : "hidden"
              }`}
            >
          ...
          <div onClick={() => setSidebarOpen(false)}>
            <Link
              to={url_var}
              className={`flex item-center px-6 py-3.5 -mx-2 -my-1 rounded-sm ${
                activePath(url_var)
                  ? "bg-red-700 text-white border-white"
                  : "text-gray-300 border-transparent"
              } border-l-8 hover:text-white hover:border-white hover:bg-red-900`}
            >
        ...
        {children && children.length > 0 ? (
          <div>
            ...
            <ul
              className={`${
                menuActive === url_var
                  ? "bg-red-800 absolute top-0 left-14 z-50 pl-2 pr-5 w-max rounded transition duration-500 ease-in-out transform hover:scale-105"
                  : "hidden"
              }`}
            >
                ...
                <li key={key}>
                  <Link
                    ...
                    className={`px-4 py-2.5 rounded-lg my-1 ${
                      activePath(url_var)
                        ? "text-white bg-red-900 rounded -mr-3"
                        : "text-gray-300"
          ...
          <Link
            ...
            className={`${
              activePath(url_var)
                ? "bg-white text-red-800 scale-150"
                : "text-white"
            } w-6 h-6 rounded flex justify-center items-center hover:bg-white mb-3 hover:text-red-800 transition duration-200 ease-in-out transform hover:scale-150`}
          >
  ...
  return (
    <>
      <div className={`z-20`}>
        {/* SIDEBAR MOBILE */}
        <div
          className={`fixed inset-0 bg-black bg-opacity-60 z-40 md:hidden md:z-auto transition-opacity duration-200 ${
          ...
          >
            <div className="cursor-pointer z-10 w-full px-4 mb-8 mt-6 text-center">
              <span className="mt-2 cursor-pointer">
                {collapse ? "Quillaborate" : "Quill"}
              </span>
            </div>

            {collapse ? (
              <div className="w-full space-y-4 px-1.5">{getRoutesPath()}</div>
            ) : (
