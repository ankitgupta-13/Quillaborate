...
const OtpPage = () => {
  ...
  const phoneNumber =
    JSON.parse(localStorage.getItem("doc-regist"))?.phone_number_int ||
    "6281234567890";
        ...
        <div className="relative bg-white shadow-lg sm:rounded-3xl px-4 py-10 md:p-20">
          <div>
            <h1 className="text-xl font-semibold text-center mb-5">
              Enter your verfication code to continue
            </h1>
          </div>
          <div className="md:max-w-md mx-auto flex flex-col items-center">
            <p className="text-center text-sms mb-8 w-9/12">
              Verification code has been sent via WhatsApp to{" "}
              <span className="font-bold">{hashNumber(phoneNumber)}</span>
            </p>
            <OtpInput
            ...
            {!success && (
              <button
                ...
                } text-sms text-white mt-8 w-11/12 py-2.5 rounded`}
              >
                Verify
              </button>
            )}
            {success && (
              <p className="text-sms mt-7 mb-2 text-center text-red-800">
                Success verification, please login !
              </p>
            )}
            {!success && (
              <p className="text-sms mt-7 mb-2 text-center w-8/12">
                Please wait for{" "}
                <span className="font-bold">{timeLeft} seconds</span> to resend
                verification code
              </p>
            )}
            {!success && timeLeft === 0 && (
              <p
                className="text-red-800 hover:text-red-600 text-sms mt-3 cursor-pointer font-semibold"
