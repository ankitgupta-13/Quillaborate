...
const inputText =
  "outline-none border-1 border-gray-200 rounded-lg py-2 px-3 sm:p-3 mt-1 focus:ring-1 focus:ring-red-800 focus:border-red-800";
...
const DocumentDetail = ({ user, setCollapse }) => {
  ...
  useEffect(() => {
    if (pdfViewer) {
      ...
      pdfViewer.downloadFileName =
        dataDocument?.c_document_code || new Date().getTime();
    }
  ...
  //               } else {
  //                 toast.error(
  //                   `${res.config?.url} ${res.status} ${res.statusText}`
  //                 );
  //               }
  //               showLoader(false);
  //             }
  ...
  return (
    <div className="flex item-center flex-col py-5 px-3 md:p-6 mb-auto">
      <div className="flex items-center justify-between">
        <button
          className="w-9 h-9 md:w-10 md:h-10 bg-red-800 text-white hover:bg-red-600 shadow rounded-full flex justify-center items-center text-3xl transition duration-200 ease-in-out transform hover:scale-110"
          onClick={() => navigate(-1)}
        >
      ...
      <div className="flex justify-center items-center flex-col bg-white rounded-2xl shadow p-6 md:p-4 mt-5 font-medium text-center">
        ...
        <p className="font-semibold text-base mt-5">
          {dataDocument?.c_document_code || "Document Code"}
        </p>
        <p className="font-bold text-xl mt-2 w-9/12 md:w-1/2">
          {dataDocument.title || "Document Title"}
        </p>
        <p className="mt-2 w-9/12 md:w-1/2">
          {dataDocument?.data || "Description of the document"}
        </p>
        <p className="my-3">
          Created on{" "}
          {moment(dataDocument?.d_created_at).format("DD MMM YYYY HH:mm") ||
        ...
        <div className="my-2 w-full">
          <div className="flex justify-center items-center">
            ...
            <div
              className={`ml-2 w-8 h-8 rounded-full flex justify-center items-center ${
                dataDocument?.i_current_stat / dataDocument?.q_contributor === 1
                  ? "bg-succes"
                  : "bg-danger"
              }`}
            >
              <div className="w-5.5 h-5.5 bg-white rounded-full flex justify-center items-center text-xsm">
                {(
                  (100 * dataDocument?.i_current_stat) /
                  dataDocument?.q_contributor
                ).toFixed(0)}
                %
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row justify-evenly items-center mt-2">
            {dataDocument?.detail?.length > 0 &&
              dataDocument.detail.map((data, key) => (
                <div key={key} className="flex flex-col items-center">
                  <div
                    className={`mb-1 flex justify-center items-center w-20 h-20 rounded-full ${
                      data.b_approve ? "bg-succes" : "bg-danger"
                    } ${
                      parseInt(data.i_stat) ===
                        dataDocument?.i_current_stat + 1 && "progress"
                    }`}
                  >
                    <div className="bg-white w-13 h-13 rounded-full flex justify-center items-center text-base font-semibold">
                      {key + 1}
                    </div>
                  </div>
      ...
      {user.i_group !== 1 ? (
          ...
          {dataDocument?.e_encode_document && (
            <PdfViewerComponent
              ...
              toolbarSettings={{
                ...
                formDesignerToolbarItems: formDesignerToolbarItems,
                showTooltip: true,
              }}
            >
          ...
          {/* <form className="mt-8">
            <div className={containerInput}>
              ...
              <input
                type="text"
                className={inputText}
                placeholder="Note to the document"
                {...register("c_note", { required: "Note is required." })}
              />
        ...
        <div className="bg-white rounded-2xl shadow p-6 mt-5 h-full">
          <embed
            src={dataDocument?.e_encode_document}
            type="application/pdf"
            className="w-full h-screen"
            title={dataDocument?.c_document_code}
          />
        </div>
      )}
